...
public class Main {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; (choice.equals("yes")) ; i++) {
			...
			do {
				System.out.printf("Enter %s quantity:\n", item[i].getItemName());
				int quantity = Integer.parseInt(sc.nextLine());
				if (item[i].setQuantity(quantity)) {
					netQuantity = netQuantity + item[i].getQuantity();	 //get the current quantity and add to total
					break;
				} else {
			...
			netTotal = netTotal + subTotal;				//Add the value to netTotal
		...
		// for loop for printing all data
